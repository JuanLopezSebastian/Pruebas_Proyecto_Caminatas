...
public class GaleriaController {
    ...
    @PostMapping
    public ResponseEntity<GaleriaEntity> create(@Valid @RequestBody GaleriaDTO dto) {
        GaleriaEntity galeria = galeriaService.save(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(galeria);
    }

    @Operation(summary = "Listar todas las galerías")
    @GetMapping
    public ResponseEntity<List<GaleriaEntity>> findAll() {
        return ResponseEntity.ok(galeriaService.findAll());
    }

    @Operation(summary = "Buscar galería por ID")
    @GetMapping("/{id}")
    public ResponseEntity<GaleriaEntity> findById(@PathVariable Long id) {
