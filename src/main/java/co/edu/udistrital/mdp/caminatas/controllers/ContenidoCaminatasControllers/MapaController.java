...
public class MapaController {
    ...
    @PostMapping
    public ResponseEntity<MapaEntity> crearMapa(@Valid @RequestBody MapaDTO dto) {
        MapaEntity creado = mapaService.save(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(creado);
    }

    @Operation(summary = "Obtener todos los mapas")
    @GetMapping
    public ResponseEntity<List<MapaEntity>> obtenerTodos() {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<MapaEntity> obtenerPorId(@PathVariable Long id) {
